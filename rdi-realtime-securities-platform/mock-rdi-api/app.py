"""
Mock RDI Control-Plane API for the HDFC Securities demo.
...
JWT_SECRET     = os.getenv("RDI_JWT_SECRET", "hdfcsec-rdi-demo-secret")
...
def statistics():
    ...
    for name, src in (cfg.get("sources") or {}).items():
        ...
        connections[name] = {
            ...
            "database": c.get("database", "hdfcsec"),
            "user": "***",
            "password": "***",
            "status": "connected",
        }
