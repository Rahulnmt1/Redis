# Redis Data Integration (RDI) — HDFC Securities Demo

A live demo of **Redis Data Integration** built around HDFC Securities'
portfolio use case. The architecture is designed for crore-class
...
> **Why PostgreSQL?** HDFC Sec runs Oracle in production. RDI supports
...
> master) — which is what every real HDFC Sec API hits — is firmly in
...
**Beyond a laptop** — for real HDFC Sec workloads with crore-class
...
> logs hdfcsec-redis-enterprise` shows `sentinel_service` exiting with
...
>
> ```bash
> docker compose ps                # all 8 hdfcsec-* services should be Up / healthy
...
> isn't up yet — check `docker logs hdfcsec-<service>` and see
...
| <https://localhost:8443> | Target Redis Enterprise cluster UI (**Cluster Manager**) | `admin@hdfcsec.demo` | `HDFCsecRedis!1` |
| <https://localhost:8444> | RDI-state Redis Enterprise cluster UI (**Cluster Manager**) | `admin@hdfcsec.demo` | `HDFCsecRedis!1` |
...
|---|---|---|---|
| `https://localhost:9443/v1/...` | Target RE cluster — REST API | `admin@hdfcsec.demo` | `HDFCsecRedis!1` |
| `https://localhost:9444/v1/...` | RDI-state RE cluster — REST API | `admin@hdfcsec.demo` | `HDFCsecRedis!1` |

Example: `curl -sk -u 'admin@hdfcsec.demo:HDFCsecRedis!1' https://localhost:9443/v1/bdbs | jq`
...
|---|---|---|---|---|
| `localhost:15432` | **Admin / app user** — used by the dashboard's latency comparison and by `scripts/seed_large_scale.py` | `postgres` | `postgres` | `hdfcsec` |
| `localhost:15432` | **CDC replication user** — used by Debezium to read the WAL (you should not need this manually) | `rdi_cdc_user` | `rdi_cdc_pwd` | `hdfcsec` |
...
> has no `hdfcsec` database — and you'd see `FATAL: database
> "hdfcsec" does not exist`. Publishing the container on **15432**
...
Example: `psql -h localhost -p 15432 -U postgres -d hdfcsec -c "SELECT count(*) FROM portfolio.customer;"`
...
```bash
docker port hdfcsec-postgres 5432        # expect: 0.0.0.0:15432
...
5. Click **Finish**. In the Database Navigator, expand
   **hdfcsec → Schemas → portfolio** to see the five demo tables:
   `customer`, `holding`, `trade`, `security_master`, `market_price`.

> **If `Test Connection…` returns `FATAL: database "hdfcsec" does
...
```sql
select * from hdfcsec.portfolio.customer;
select count(*) from hdfcsec.portfolio.customer;

select * from hdfcsec.portfolio.holding;
select count(*) from hdfcsec.portfolio.holding;

select * from hdfcsec.portfolio.market_price;
select count(*) from hdfcsec.portfolio.market_price;

select * from hdfcsec.portfolio.security_master;
select count(*) from hdfcsec.portfolio.security_master;

select * from hdfcsec.portfolio.trade;
select count(*) from hdfcsec.portfolio.trade;
...
FROM hdfcsec.portfolio.customer c
JOIN hdfcsec.portfolio.holding  h USING (customer_id)
...
```
jdbc:postgresql://localhost:15432/hdfcsec
```
...
| **Step-by-step trace of the last fired event** | A staged timeline of the row's journey: PostgreSQL INSERT/UPDATE · WAL commit · Debezium pickup · RDI processor write to Redis · application read — each stamped with an IST timestamp and the per-stage delta | proves CDC pickup is **sub-ms**, RDI processor → Redis is **~4 ms**, and the application read is **~100 µs** — the numbers HDFC Sec's architecture team wants to see |
...
- **Redis read latency** — what HDFC Sec's trading app actually pays
...
Every card has a **For HDFC Sec:** sentence explaining the business
...
Each card frames an RDI capability as a concrete HDFC Securities
...
```bash
# pre-check
docker exec hdfcsec-redis-enterprise redis-cli -p 12000 DBSIZE
# expected: >= 40
```
...
```bash
docker exec -it hdfcsec-postgres psql -U postgres hdfcsec
```
...
Stream), and 5 CDC streams under `hdfcsec.portfolio.*` in the RDI
...
docker logs --tail 30 hdfcsec-debezium | grep -i "redis\|snapshot"

# Confirm the RDI processor is consuming
docker logs --tail 30 hdfcsec-rdi-processor
...
| Symptom | Fix |
|---|---|
| `setup.sh` hangs on "Waiting for Redis Enterprise bootstrap" | First boot of Redis Enterprise takes ~60 s. Check `docker logs hdfcsec-re-bootstrap` |
| Dashboard shows "customer not found" | Initial snapshot not finished. `docker logs -f hdfcsec-rdi-processor` should show `JSON.SET` entries |
...
| Insight RDI tab: `ECONNREFUSED ...:443` | The `rdi-api` service isn't up. `docker compose up -d rdi-api && docker logs hdfcsec-rdi-api` |
