...
# ---------- 2. Containers --------------------------------------------
hdr "2. Containers"
expected=(hdfcsec-postgres hdfcsec-redis-enterprise hdfcsec-redis-rdi \
          hdfcsec-debezium hdfcsec-rdi-processor hdfcsec-rdi-api \
          hdfcsec-redis-insight hdfcsec-dashboard)
...
streams=(hdfcsec.portfolio.customer hdfcsec.portfolio.holding \
         hdfcsec.portfolio.trade hdfcsec.portfolio.security_master \
         hdfcsec.portfolio.market_price)
for s in "${streams[@]}"; do
  groups=$(docker exec hdfcsec-redis-rdi redis-cli -p 12001 \
...
demo_present=$(docker exec hdfcsec-redis-enterprise redis-cli -p 12000 \
...
dbsize=$(docker exec hdfcsec-redis-enterprise redis-cli -p 12000 DBSIZE 2>/dev/null)
...
slot_out=$(docker exec hdfcsec-postgres psql -U postgres -d hdfcsec -tA \
