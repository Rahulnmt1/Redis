#!/usr/bin/env bash
# =====================================================================
# One-command setup for the Securities & Trading Firm RDI demo.
...
echo "================================================================="
echo " Securities & Trading Firm - Redis Data Integration (RDI) Demo"
echo "================================================================="
...
for _ in $(seq 1 120); do
  state=$(docker inspect -f '{{.State.Status}}' sectrade-re-bootstrap 2>/dev/null || echo missing)
  code=$(docker inspect  -f '{{.State.ExitCode}}' sectrade-re-bootstrap 2>/dev/null || echo 1)
...
for _ in $(seq 1 30); do
  c=$(docker exec sectrade-redis-rdi sh -c \
        "redis-cli -p 12001 --scan --pattern 'sectrade.portfolio.*' | wc -l" \
...
if [ "$ok" != "1" ]; then
  echo "      WARN: only $c/5 CDC streams visible after 90s — check 'docker logs sectrade-rdi-processor'"
fi
...
=================================================================
 ...
 Once the seeder + health-check finish, open these in your browser:
  ...
  Target RE Cluster UI  :  https://localhost:8443   (admin@sectrade.demo / SecTradeRedis!1)
  RDI    RE Cluster UI  :  https://localhost:8444   (admin@sectrade.demo / SecTradeRedis!1)
