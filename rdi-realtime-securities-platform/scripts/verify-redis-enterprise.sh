...
RE_USER="admin@hdfcsec.demo"
RE_PASS="HDFCsecRedis!1"
...
re_img=$(docker inspect hdfcsec-redis-enterprise --format '{{.Config.Image}}' 2>/dev/null || echo missing)
rdi_img=$(docker inspect hdfcsec-redis-rdi --format '{{.Config.Image}}' 2>/dev/null || echo missing)
ins_img=$(docker inspect hdfcsec-redis-insight --format '{{.Config.Image}}' 2>/dev/null || echo missing)
deb_img=$(docker inspect hdfcsec-debezium --format '{{.Config.Image}}' 2>/dev/null || echo missing)
...
modules=$(docker exec hdfcsec-redis-enterprise redis-cli -p 12000 MODULE LIST 2>/dev/null | tr '\n' ' ')
...
qty=$(docker exec hdfcsec-redis-enterprise redis-cli -p 12000 \
...
multi=$(docker exec hdfcsec-redis-enterprise redis-cli -p 12000 \
...
for t in "${expected_streams[@]}"; do
  l=$(docker exec hdfcsec-redis-rdi redis-cli -p 12001 XLEN "hdfcsec.portfolio.$t" 2>/dev/null)
  if [[ "${l:-0}" -gt 0 ]]; then
    pass "stream hdfcsec.portfolio.$t  len=$l"
  else
    fail "stream hdfcsec.portfolio.$t  missing or empty"
  fi
done

hdr "6. Debezium engine identity"
docker logs hdfcsec-debezium 2>&1 | grep -q RedisStreamChangeConsumer \
  && pass "RedisStreamChangeConsumer instantiated (real RDI sink)" \
  || fail "Redis sink not found in Debezium logs"
docker logs hdfcsec-debezium 2>&1 | grep -q "Engine executor started" \
...
writes=$(docker logs hdfcsec-rdi-processor 2>&1 | grep -cE 'JSON\.SET|HSET|XADD')
[[ $writes -gt 30 ]] && pass "$writes write commands issued (JSON.SET / HSET / XADD)" \
  || fail "only $writes write commands seen"
echo "  note: hdfcsec/rdi-processor:demo is a REFERENCE IMPLEMENTATION,"
...
oid="PROOF-$(date +%s)"
docker exec hdfcsec-postgres psql -U postgres -d hdfcsec -tA -c \
...
sleep 3
in_state=$(docker exec hdfcsec-redis-rdi redis-cli -p 12001 \
             XREVRANGE hdfcsec.portfolio.trade + - COUNT 1 | grep -c "$oid")
in_target=$(docker exec hdfcsec-redis-enterprise redis-cli -p 12000 \
...
=================================================================
 ALL CHECKS PASSED
   ...
   REFERENCE IMPLEMENTATIONS in this stack (NOT Redis-distributed):
     - hdfcsec/rdi-processor:demo  - mimics real RDI's stream
                                     processor; YAML jobs are
                                     verbatim-portable to real RDI
     - hdfcsec/mock-rdi-api:demo   - mimics RDI control-plane
