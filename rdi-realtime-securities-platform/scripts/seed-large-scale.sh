...
if ! docker ps --format '{{.Names}}' | grep -q '^sectrade-dashboard$'; then
  echo "FATAL: sectrade-dashboard container is not running."
  echo "       Run ./scripts/setup.sh first."
  exit 1
fi

# Copy the seeder INTO the dashboard container so it shares the docker-network DNS
docker cp scripts/seed_large_scale.py sectrade-dashboard:/tmp/seed_large_scale.py
...
docker exec \
  ...
  sectrade-dashboard python /tmp/seed_large_scale.py
