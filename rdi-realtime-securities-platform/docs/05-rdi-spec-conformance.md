...
> fail to materialise when the firm moves to a real RDI install in their
> PoC.
>
> **Source of truth**:
...
```yaml
# customer.yaml
source:    { schema: portfolio, table: customer, server_name: sectrade }
...
```yaml
# holding.yaml
source:    { schema: portfolio, table: holding, server_name: sectrade }
...
```yaml
# trade.yaml
source:    { schema: portfolio, table: trade, server_name: sectrade }
...
```yaml
# market_price.yaml
source:    { schema: portfolio, table: market_price, server_name: sectrade }
...
source:    { schema: portfolio, table: security_master, server_name: sectrade }
...
The custom Python reference processor (`sectrade/rdi-processor:demo`) does
...
(`sectrade-redis-enterprise` and `sectrade-redis-rdi`), each with its own
...
When the firm installs real RDI in their environment, RDI's installer
