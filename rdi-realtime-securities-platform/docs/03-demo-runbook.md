# Demo Runbook — HDFC Securities RDI Demo
...
docker exec hdfcsec-postgres psql -U postgres -d hdfcsec -c \
  "SELECT COUNT(*) AS customers FROM portfolio.customer;"
# expected: 10

# 2. Debezium running
docker logs hdfcsec-debezium 2>&1 | tail -20 | grep -i "Snapshot ended"
# expected: at least one match within a minute of start

# 3. RDI processor consuming
docker logs hdfcsec-rdi-processor 2>&1 | tail -20 | grep -c "JSON.SET\|HSET"
# expected: >20
...
# expected: {"note":"Demo-only mock. ...","service":"hdfcsec-mock-rdi-api","version":"..."}
...
1. **Slide deck**: `file:///Users/rahul.choubey/Downloads/RDI-Demo-HDFCSEC/docs/01-slide-deck.html` (or open via Finder)
...
|---|---|---|
| 1 | psql session for live SQL | `docker exec -it hdfcsec-postgres psql -U postgres hdfcsec` |
| 2 | Simulation runner | `./scripts/run-simulation.sh` (don't execute yet) |
| 3 | Spare / logs | `docker logs -f hdfcsec-rdi-processor` |
...
| Slides | Purpose |
|---|---|
| 1–9   | Problem framing, RDI architecture, HDFC mapping, demo center |
...
> "Top header — what the components are. Left rail — list of HDFC Sec
...
```bash
docker logs --tail 30 hdfcsec-rdi-processor   # look for JSON.SET
...
- The **RDI pipeline streams** card shows the
  `hdfcsec.portfolio.trade` event count went up by 1.
...
```bash
docker logs --tail 20 hdfcsec-rdi-processor
docker logs --tail 20 hdfcsec-debezium
```
...
```bash
docker exec hdfcsec-redis-rdi redis-cli -p 12001 XLEN \
  hdfcsec.portfolio.trade
```
...
- RDI state DB has 5 CDC streams under `hdfcsec.portfolio.*`.
...
| Filter | What to show |
|---|---|
| `hdfcsec.portfolio.*` | 5 streams, one per source table |
...
2. `docker exec hdfcsec-redis-enterprise redis-cli -p 12000 DBSIZE`
   should be ≥ 40.
3. If 0: `docker logs hdfcsec-rdi-processor` — look for connection
   errors. Usually means Debezium hasn't snapshotted yet.

### Live trade doesn't appear within 5s

1. Check Debezium: `docker logs hdfcsec-debezium | tail -20` — should
   show stream activity.
2. Check processor: `docker logs hdfcsec-rdi-processor | tail -10` —
...
docker logs --tail 20 hdfcsec-rdi-api
