"""
HDFC Securities · RDI Capability Showcase Dashboard
====================================================
...
the headline is now an interactive map of RDI capabilities and HDFC
Sec use-cases.
...
Production note: in HDFC Sec's live RDI install, all of the DDL /
...
# HDFC Sec engineers see them in their own timezone without needing to do
...
def pipeline_metrics() -> dict:
    ...
    out = OrderedDict()
    try:
        for s in rdi_state.scan_iter(match="hdfcsec.portfolio.*",
...
#    1. The SQL that the HDFC app WOULD run against Postgres (plus its p50 ms)
...
#   3. RDI · Debezium picked it up      (observed via the hdfcsec.portfolio.*
#                                        stream on rdi-state)
...
def api_pipeline_inject():
    ...
    if action in ("onboard", "customer"):
        ...
        stream  = "hdfcsec.portfolio.customer"
    ...
    if action == "trade":
        ...
        stream = "hdfcsec.portfolio.trade"
        target = f"trades:{cid}"
    ...
    if action == "holding":
        ...
        stream  = "hdfcsec.portfolio.holding"
        since   = _last_stream_id(stream)
    ...
    _COLUMN_UPDATE_ACTIONS = {
        ...
        "instrument_status": {
            ...
            "stream": "hdfcsec.portfolio.security_master",
            "pk_col": "security_id",
            "col":    "is_active",
            "values": ("toggle_bool",),
        ...
        "contact_update": {
            ...
            "values": (
                "raj.demo+a@hdfcsec.demo",
                "raj.demo+b@hdfcsec.demo",
                "priya.demo+a@hdfcsec.demo",
                "priya.demo+b@hdfcsec.demo",
                "amit.demo+a@hdfcsec.demo",
            ),
            "target_key_tpl": "customer:{client_code}",
            "json_path":      "$.email",
...
# Each use-case wraps one-or-more capabilities into an HDFC-specific demo.
# ===========================================================================
...
def api_uc_settle_trade():
    ...
    grow AND the holding update propagate. Mirrors what HDFC Sec's
...
def api_uc_risk_margin():
    ...
    materialised-margin scenario for HDFC Sec's risk engine."""
    cc = (request.json or {}).get("client_code") if request.is_json else None
    if not cc:
        cc = _pick_verified_client_code()
    cust = _pg_select_one(
...
_SYNC_TABLES = [
    ...
    },
    {
        ...
        "stream":     "hdfcsec.portfolio.market_price",
